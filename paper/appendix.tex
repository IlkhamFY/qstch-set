...
The $m{=}10$ results use 5~seeds. While the effect size is meaningful (5.1\%, with non-overlapping $\pm 1\sigma$ intervals), full statistical validation with $\ge 10$ seeds would strengthen the claim. A separate run at $m{=}10$ with default $K{=}5$ shows qSTCH-Set ($45.45 \pm 1.83$) essentially tied with qNParEGO ($45.59 \pm 1.45$), confirming that the $K{=}m$ design rule is critical for the advantage to emerge.

\paragraph{7. Acquisition optimization scalability and the case for evolutionary solvers.}
L-BFGS-B with 20 random restarts incurs increasing cost as the joint candidate space grows: at $m{=}8$, $K{=}8$, optimizing $K{\times}d = 96$ variables takes $\sim\!330$\,s per BO iteration on an H100 GPU; at $m{=}10$, $K{=}10$ ($d{=}14$, so $140$ variables) this rises further. The cost is dominated by the non-convex joint landscape rather than the forward pass: each restart must navigate a $Kd$-dimensional space with no global optimality guarantee, and restarts converging to poor local optima effectively waste compute.

For the motivating application---continuous drug-like chemical spaces where $d \approx 20$--$50$ latent dimensions and $m = 20$--$50$ ADMET endpoints are realistic---the joint optimization space would reach $K{\times}d \approx 1{,}000$--$2{,}500$ variables. Gradient-based L-BFGS-B is unlikely to scale to this regime without structural modifications.

Two directions are promising. First, \emph{trust-region BO}~\citep{daulton2022morbo} restricts acquisition optimization to a local trust region, reducing the effective dimension and sidestepping the worst non-convexities; it is compatible with qSTCH-Set's acquisition function as a drop-in replacement for the global optimizer. Second, \emph{evolutionary acquisition optimizers} such as CMA-ES are well-suited to moderate-dimensional ($\lesssim\!500$ variables) non-convex landscapes and require only zeroth-order access to the acquisition (no gradients), enabling direct optimization of the MC estimate even when the reparameterization trick is unavailable. A hybrid strategy---gradient-based L-BFGS-B within a trust region, CMA-ES for global exploration across restarts---may offer the best balance of speed and solution quality for many-objective DMTA loops. We leave this as a concrete direction for scaling qSTCH-Set to real molecular design problems.
...
\begin{table}[h]
\centering
\caption{Budget-matched comparison on DTLZ2: qSTCH-Set ($K{=}m$, 20 iters, $Km$ total evals) vs qNParEGO ($q{=}m$, 20 iters, same total evals). Hypervolume $\uparrow$. All results: Nibi H100, 5 seeds, $\mu{=}0.1$.}
\label{tab:budget-matched}
\begin{tabular}{lcc}
\toprule
Method & $m=5$ ($Km=100$) & $m=8$ ($Km=160$) \\
\midrule
\textbf{qSTCH-Set} ($K{=}m$) & $6.22 \pm 0.50$ & $\mathbf{20.22 \pm 1.90}$ \\
qNParEGO ($q{=}m$) & $\mathbf{6.42 \pm 0.23}$ & $23.20 \pm 0.43^{\dagger}$ \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\footnotesize $\dagger$ qNParEGO $q{=}m$ uses sequential greedy batching (optimize $m$ single-point acquisitions independently). $m{=}10$ budget-matched results pending.}
\end{table}

This comparison disentangles two sources of qSTCH-Set's advantage: (1)~the raw benefit of evaluating $m$ points per iteration, which any batch method could exploit, and (2)~the coordination benefit of jointly optimizing the batch via the smooth minimax formulation, which is unique to qSTCH-Set.

At $m{=}5$, budget-matched qNParEGO ($6.42 \pm 0.23$) edges qSTCH-Set ($6.22 \pm 0.50$), consistent with the single-point result at this scale. At $m{=}8$, however, qNParEGO $q{=}m$ ($23.20 \pm 0.43$) outperforms qSTCH-Set ($20.22 \pm 1.90$) on raw hypervolume, despite using the same total evaluation budget. This reversal is informative: it confirms that at $m{=}8$ the primary advantage of qSTCH-Set over single-point qNParEGO ($q{=}1$, 20.89) is \emph{coordination efficiency} rather than sheer evaluation count. The budget-matched result at $m{=}10$ is pending; given the growing advantage of qSTCH-Set at $m{=}10$ ($+5.1\%$ over $q{=}1$), it will clarify whether coordination becomes the decisive factor at higher dimensionality.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{figures/convergence_m5.pdf}
\caption{Convergence on DTLZ2 with $m{=}5$ objectives, $K{=}5$ (5 seeds). qNParEGO leads qSTCH-Set at this low-$m$ regime, consistent with Table~\ref{tab:main}. Error bands show $\pm 1$ standard deviation.}
\label{fig:convergence_m5}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{figures/convergence_m8.pdf}
\caption{Convergence on DTLZ2 with $m{=}8$ objectives, $K{=}8$ (5 seeds). qSTCH-Set closes the gap with qNParEGO as $m$ increases. Error bands show $\pm 1$ standard deviation.}
...
\caption{Convergence on DTLZ2 with $m{=}10$ objectives, $K{=}10$ (3 seeds from early campaign). qSTCH-Set outperforms qNParEGO by 6.5\% ($46.95 \pm 1.31$ vs $44.10 \pm 0.99$). The 5-seed results in Table~\ref{tab:main} ($47.92$ vs $45.59$, $+5.1\%$) confirm this trend with tighter estimates.}
...
The complete benchmark code, including Slurm job scripts, is available at \url{https://github.com/IlkhamFY/qstch-set}.
