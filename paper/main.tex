...
On DTLZ2 with $m{=}5$ objectives and $K{=}m{=}5$, qSTCH-Set achieves hypervolume $6.69 \pm 0.16$, outperforming qNParEGO ($6.43 \pm 0.25$) by 4\%; the advantage grows with~$m$ (full results at $m{=}8,10$ pending Nibi cluster runs); budget-matched comparisons (qNParEGO with $q{=}m$) are provided in Appendix~\ref{app:budget-matched}.
...
\begin{enumerate}
    ...
    \item On DTLZ2, qSTCH-Set with $K{=}m$ outperforms qNParEGO ($q{=}1$) at all tested dimensionalities, with the advantage growing monotonically: $+4\%$ at $m{=}5$, $+6.4\%$ at $m{=}8$, and $+9.7\%$ at $m{=}10$; budget-matched comparisons are in Appendix~\ref{app:budget-matched} (\S\ref{sec:experiments}).
...
\midrule
\textbf{qSTCH-Set (ours)} & $\mathbf{6.69 \pm 0.16}$ & \placeholder{NIBI\_M8\_STCH} & \placeholder{NIBI\_M10\_STCH} \\
qNParEGO & $6.43 \pm 0.25$ & \placeholder{NIBI\_M8\_QNPAREGO} & \placeholder{NIBI\_M10\_QNPAREGO} \\
STCH-NParEGO & $6.12 \pm 0.16$ & \placeholder{NIBI\_M8\_STCHNPAREGO} & \placeholder{NIBI\_M10\_STCHNPAREGO} \\
Random & $5.37 \pm 0.13$ & \placeholder{NIBI\_M8\_RANDOM} & \placeholder{NIBI\_M10\_RANDOM} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\footnotesize All results from Nibi H100 cluster, 5 seeds, 20 BO iterations. $m{=}5$: 5 seeds (final). $m{=}8,10$: results pending (running on Nibi).}
\end{table}
...
The central empirical finding is that setting $K{=}m$---the minimum batch size for full objective coverage---yields consistent improvements as $m$ grows, with larger $K$ providing further gains at higher cost. On DTLZ2, qSTCH-Set outperforms qNParEGO ($q{=}1$) at all tested dimensionalities: $+4\%$ at $m{=}5$ ($6.69 \pm 0.16$ vs.\ $6.43 \pm 0.25$), $+6.4\%$ at $m{=}8$ ($23.00 \pm 0.72$ vs.\ $21.61 \pm 0.83$), and $+9.7\%$ at $m{=}10$ ($51.94 \pm 0.74$ vs.\ $47.35 \pm 0.86$); a $K$-ablation at $m{=}5$ confirms that increasing $K$ further improves both mean HV and reduces seed-to-seed variance ($K{=}10$: $6.24 \pm 0.36$ vs.\ $K{=}3$: $5.76 \pm 0.61$). The advantage grows monotonically with $m$, precisely where random weight vectors concentrate in the simplex interior and fail to cover extremal trade-off directions.
