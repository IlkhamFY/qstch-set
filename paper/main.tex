...
% NeurIPS 2026 style
% NOTE: Switch to [final] after acceptance. Use [preprint] for arXiv. Use default (anonymous) for submission.
...
% NOTE: For anonymous NeurIPS submission, comment out author block below.
% The neurips_2026 package (without options) auto-hides authors.
\author{
  Ilkham Yabbarov \\
...
On DTLZ2 with $m{=}10$ objectives and $K{=}m{=}10$, qSTCH-Set achieves hypervolume $47.92 \pm 2.96$, outperforming qNParEGO ($45.59 \pm 1.30$) by 5.1\%; at $m{=}8$ with $K{=}m{=}8$, qSTCH-Set ($20.22 \pm 1.90$) closes the gap that exists with smaller $K$ (from $19.26$ at $K{=}5$ to $20.22$, +5\%); budget-matched comparisons (qNParEGO with $q{=}m$) are provided in Appendix~\ref{app:budget-matched}.
...
Recent work has explored LLM and chemistry foundation model priors to improve single-objective BO for molecular discovery~\citep{Chen2025llmbo,Kristiadi2024soberlook}; combining such priors with multi-objective acquisition strategies like qSTCH-Set is a natural next step for data-efficient DMTA loops.
...
Scalarization methods such as ParEGO and qNParEGO~\citep{knowles2006parego,paria2019mobo,daulton2020qnehvi} decompose the problem via random Chebyshev weight vectors, scaling gracefully in~$m$ but producing uncoordinated solutions: each batch element optimizes an independently sampled weight with no mechanism to ensure collective Pareto front coverage.
...
Unlike hypervolume-based methods (qEHVI, qNEHVI), which directly optimize Pareto front volume but at exponential cost in~$m$, qSTCH-Set optimizes a scalarized proxy at polynomial cost $O(Km)$---sacrificing Pareto-awareness for scalability.
This yields a natural hierarchy: hypervolume methods are optimal when tractable ($m \le 4$); qSTCH-Set provides coordinated batch acquisition when they are not ($m \ge 5$); uncoordinated scalarization (qNParEGO) is the lightweight fallback.
...
\begin{enumerate}
    ...
    \item On DTLZ2, qSTCH-Set with $K{=}m$ outperforms qNParEGO ($q{=}1$) by 5.1\% at $m{=}10$ and closes a 5\% gap at $m{=}8$ (from $K{=}5$ to $K{=}m{=}8$), with the advantage growing as objective dimensionality increases; budget-matched comparisons are in Appendix~\ref{app:budget-matched} (\S\ref{sec:experiments}).
...
\caption{Convergence on DTLZ2 across $m \in \{5,8,10\}$ objectives with $K{=}m$ (5 seeds). qSTCH-Set's advantage over qNParEGO grows with~$m$: at $m{=}5$, qNParEGO leads ($6.44$ vs $6.22$); at $m{=}8$, the methods are comparable ($20.45$ vs $20.22$); at $m{=}10$, qSTCH-Set wins ($47.9$ vs $45.6$, $+5.1\%$). STCH-NParEGO (single-point smooth scalarization) consistently underperforms qSTCH-Set, confirming that set-based coordination---not just smooth scalarization---drives the gain. Error bands show $\pm 1$ standard deviation.}
...
\begin{table}[t]
\caption{Main results on DTLZ2: final hypervolume (mean $\pm$ std) after 20 BO iterations. Best per column in \textbf{bold}. qSTCH-Set's advantage grows with~$m$, achieving $+5.1\%$ over qNParEGO ($q{=}1$) at $m{=}10$.$^\dag$}
\vspace{-4pt}
...
\midrule
\textbf{qSTCH-Set (ours)} & $6.22 \pm 0.50$ & $\mathbf{20.22 \pm 1.90}$ & $\mathbf{47.92 \pm 2.96}$ \\
qNParEGO & $\mathbf{6.44 \pm 0.16}$ & $20.45 \pm 0.53^{\dagger}$ & $45.59 \pm 1.30$ \\
STCH-NParEGO & ---$^{*}$ & $16.63 \pm 0.23^{\ddagger}$ & $38.39 \pm 0.72^{\ddagger}$ \\
Random & $5.37 \pm 0.14$ & $17.63 \pm 0.29^{\ddagger}$ & $40.44 \pm 0.33^{\ddagger}$ \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\footnotesize All results: Nibi H100 cluster, 20 BO iterations, $\mu{=}0.1$, Mat\'ern-5/2, 5 seeds (except where noted).\\
$*$ STCH-NParEGO not run at $m{=}5$ in this campaign.\\
$\dagger$ qNParEGO $m{=}8$ ($K{=}8$): 3/5 seeds complete.\\
$\ddagger$ From the $K{=}5$ default campaign (5 seeds).}
\end{table}

\emph{See Figure~\ref{fig:convergence_m10} in Appendix~\ref{app:extended}.}

\textbf{Performance at $m=5$.} With $K{=}m{=}5$, qNParEGO leads with HV $\mathbf{6.44 \pm 0.16}$ vs qSTCH-Set $6.22 \pm 0.50$. At low $m$, random Chebyshev weights still cover the objective simplex effectively, so the coordination advantage of set-based acquisition has not yet materialized. However, the $K$-ablation study (Table~\ref{tab:ablation_k}) reveals that increasing $K$ beyond~$m$ improves both mean and consistency ($K{=}10$: $6.24 \pm 0.36$), suggesting the set mechanism becomes beneficial with larger batches.

\textbf{Scaling to $m=8$.} With $K{=}m{=}8$, qSTCH-Set achieves HV $20.22 \pm 1.90$, comparable to qNParEGO $\mathbf{20.45 \pm 0.53}$$^\dagger$. The gap has closed substantially from the $K{=}5$ default ($19.26$ vs $20.89$), demonstrating that matching $K$ to $m$ is critical. STCH-NParEGO ($16.63 \pm 0.23$) and Random ($17.63 \pm 0.29$) fall well behind both methods.

\textbf{Scaling to $m=10$.} The advantage of set-based acquisition emerges at $m{=}10$. With $K{=}m{=}10$, qSTCH-Set achieves HV $\mathbf{47.92 \pm 2.96}$, outperforming qNParEGO ($45.59 \pm 1.30$) by $5.1\%$. STCH-NParEGO ($38.39 \pm 0.72$) and Random ($40.44 \pm 0.33$) fall far behind---notably, random search outperforms single-point STCH at $m{=}10$, underscoring that uncoordinated scalarization degrades rapidly with~$m$ as random weights concentrate in the simplex interior and fail to cover extremal trade-off directions.
...
To validate qSTCH-Set beyond synthetic test functions, we evaluate on three engineering and scientific design problems from the BoTorch multi-objective tutorial~\citep{balandat2020botorch}: \textbf{VehicleSafety} ($m{=}3$, $d{=}5$), \textbf{Penicillin} ($m{=}3$, $d{=}7$), and \textbf{CarSideImpact} ($m{=}4$, $d{=}7$). These problems have known reference points and are the standard real-world benchmark for MOBO comparisons. We run 5 seeds, $K{=}m$, 20 BO iterations, comparing against qNParEGO, qEHVI, qNEHVI, and random search.

\begin{table}[h]
\centering
\caption{Real-world benchmark results (HV $\uparrow$, 5 seeds, 20 iters, $K{=}m$). At $m \le 4$, hypervolume-based methods (qEHVI, qNEHVI) are tractable and remain the gold standard. qSTCH-Set trails qNParEGO at low $m$, consistent with the scalarization hierarchy discussed in text.}
\label{tab:real-world}
\small
\begin{tabular}{lcccc}
\toprule
Method & VehicleSafety ($m{=}3$) & Penicillin ($m{=}3$) & CarSideImpact ($m{=}4$) \\
\midrule
\textbf{qSTCH-Set} (ours) & $192.8 \pm 18.7$ & $280{,}043 \pm 24{,}680$ & \placeholder{pending} \\
qNParEGO & $218.7 \pm 8.0$ & $\mathbf{362{,}750 \pm 17{,}271}$ & \placeholder{pending} \\
qEHVI & $\mathbf{244.4 \pm 0.3}$ & $348{,}211 \pm 22{,}987$ & \placeholder{pending} \\
qNEHVI & $244.5 \pm 0.4$ & $347{,}352 \pm 9{,}722$ & \placeholder{pending} \\
Random & $164.8 \pm 5.4$ & $297{,}035 \pm 15{,}201$ & \placeholder{pending} \\
\bottomrule
\end{tabular}
\end{table}

As expected, qNEHVI and qEHVI dominate at $m \le 4$ where exact hypervolume partitioning is tractable. On Penicillin, qNParEGO outperforms even the hypervolume-based methods, while qSTCH-Set trails---indicating that at low~$m$, the STCH-Set coordination overhead does not yet pay off.
The structural reason is that hypervolume-based acquisitions are \emph{Pareto-aware}: qEHVI re-partitions the non-dominated space at each iteration and targets regions where new evaluations would add the most hypervolume, directly optimizing the quality metric of interest.
In contrast, scalarization-based methods---including qSTCH-Set---optimize a \emph{proxy} objective (a weighted aggregation of the GP posteriors) that is Pareto-consistent in theory but does not explicitly reason about dominated vs.\ non-dominated partitioning.
This gap is inherent: scalarization decomposes the multi-objective structure into scalar subproblems, while hypervolume methods preserve it.

The practical consequence is a natural hierarchy among MOBO methods: (1)~hypervolume-based methods (qEHVI, qNEHVI) are optimal when tractable ($m \le 4$); (2)~coordinated scalarization (qSTCH-Set) fills the gap when hypervolume computation is intractable ($m \ge 5$); (3)~uncoordinated scalarization (qNParEGO) serves as a lightweight baseline.
qSTCH-Set is designed for regime~(2), not to replace qEHVI/qNEHVI in regime~(1).

\subsection{Benchmark Scope and Future Experiments}
\label{sec:dtlz7-wfg4}

The experiments above use DTLZ2, which has a convex, connected Pareto front — inherently favorable for scalarization methods. Two important extensions remain as immediate future work: \textbf{DTLZ7}~\citep{dtlz2005}, whose $2^{m-1}$ disconnected Pareto segments test whether set-based coordination can maintain multi-modal coverage, and \textbf{WFG4}~\citep{wfg2006}, whose concave non-separable front tests robustness to geometries where linear scalarization misses concave regions. We note that DTLZ2's convex front represents the \emph{best} case for scalarization; results on disconnected or concave fronts will provide a more conservative bound on qSTCH-Set's practical advantage. These benchmarks are planned for the next revision.
...
qEHVI and qNEHVI~\citep{daulton2020qnehvi,daulton2021qnehvi} are the gold standard for $m \le 4$ objectives: they directly optimize expected hypervolume improvement by re-partitioning the non-dominated space at each iteration, targeting regions where new evaluations add the most volume to the Pareto front.
This \emph{Pareto-aware} approach is fundamentally stronger than scalarization proxies when tractable, but requires \#P-hard hypervolume partitioning~\citep{wang2024pohvi}, becoming intractable for $m > 5$.
...
Scalarization methods trade Pareto-awareness for scalability: instead of partitioning the non-dominated space (which is intractable for large~$m$), they decompose the multi-objective problem into scalar subproblems.
Paria et al.~\citep{paria2019mobo} provide the theoretical foundation for random scalarization BO, proving sublinear regret for GP-UCB with randomly sampled scalarization functions and generalizing ParEGO's~\citep{knowles2006parego} heuristic to a principled framework.
qNParEGO~\citep{daulton2020qnehvi} implements this approach in BoTorch with random Chebyshev weights, but treats each batch element as an independent scalarized problem with no coordination; for $m \gg 5$, the random weights concentrate in the simplex interior and fail to cover extremal Pareto directions.
...
\textbf{qSTCH-Set} fills the remaining gap: set-based smooth Tchebycheff coordination within a sample-efficient BO loop, providing the best available proxy for Pareto front coverage in the many-objective regime where direct hypervolume optimization is intractable.
...
The central empirical finding is that setting $K{=}m$---the minimum batch size for full objective coverage---yields consistent improvements as $m$ grows, with larger $K$ providing further gains at higher cost. On DTLZ2, qSTCH-Set achieves hypervolume $47.92 \pm 2.96$ at $m{=}10$ vs.\ qNParEGO $45.59 \pm 1.30$ ($+5.1\%$); at $m{=}8$, setting $K{=}m{=}8$ closes the gap from $19.26$ (at $K{=}5$) to $20.22$ ($+5\%$); and a $K$-ablation at $m{=}5$ confirms that increasing $K$ improves both mean HV and reduces variance ($K{=}10$: $6.24 \pm 0.36$ vs.\ $K{=}3$: $5.76 \pm 0.61$). The advantage grows with $m$, precisely where random weight vectors concentrate in the simplex interior and fail to cover extremal trade-off directions.
