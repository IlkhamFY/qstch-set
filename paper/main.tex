...
Sorourifar et al.~\citep{paulson2025qpmhi} propose a modular ``generate-then-optimize'' framework for \emph{discrete} molecular design: a generative model first proposes a large candidate pool, after which qPMHI (Probability of Maximum Hypervolume Improvement) selects the optimal batch by ranking candidates via their probability of inducing the largest Pareto front expansion.
Because qPMHI decomposes additively over candidates, batch selection reduces to a simple sort-and-take-top-$q$ operation.
This framework is well-suited to discrete chemical spaces, where gradient-based acquisition optimization over inputs is unavailable.
In contrast, qSTCH-Set targets \emph{continuous} input spaces where end-to-end gradient flow is possible: rather than scoring a pre-generated pool, it jointly optimizes $K$ candidates through a single differentiable acquisition function, coupling them via shared GP posterior samples to ensure coordinated Pareto front coverage.
The two methods are thus complementary: qPMHI for discrete molecular libraries, qSTCH-Set for continuous BO.
