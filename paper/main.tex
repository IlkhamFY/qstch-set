...
As expected, qNEHVI and qEHVI dominate at $m \le 4$ where exact hypervolume partitioning is tractable. qSTCH-Set is competitive with qNParEGO on all three problems and substantially outperforms random search. The key point is complementarity: qSTCH-Set is designed for the many-objective regime ($m \ge 5$) where qEHVI/qNEHVI become computationally intractable, not to replace them where they work well.
