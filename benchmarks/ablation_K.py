#!/usr/bin/env python3
"""K ablation: STCH-Set on DTLZ2 m=5 with K in {2, 3, 5, 8, 10}."""
...
def run(m=5, mu=0.1, n_init=20, n_iters=20, seeds=[0,1,2], Ks=[2, 3, 5, 8, 10]):
    d = m + 1
    ...
    for K in Ks:
        ...
        for seed in seeds:
            ...
            for i in range(n_iters):
                ...
                try:
                    ...
                    print(f"  K={K} seed={seed} iter {i} error: {e}", flush=True)
                    cands = torch.rand(K, d, **tkwargs)
                    X = torch.cat([X, cands]); Y = torch.cat([Y, problem(cands)])
                hv = compute_hv(Y, ref_point)
                hv_hist.append(hv)
                print(f"  K={K} seed={seed} iter {i+1}/{n_iters}: HV={hv:.6f} ({time.time()-t0:.1f}s)", flush=True)
        ...
        results[f"K={K}"] = {"mean_hv": float(np.mean(hvs)), "std_hv": float(np.std(hvs)), "runs": seed_results}
        print(f"  K={K}: HV = {np.mean(hvs):.6f} +/- {np.std(hvs):.6f}", flush=True)
    
    with open(RESULTS_DIR / "ablation_K_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to {RESULTS_DIR / 'ablation_K_results.json'}")
